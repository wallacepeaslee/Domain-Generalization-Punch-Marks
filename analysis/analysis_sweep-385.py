...
for exp in experiments:
    ...
    jsonDataArgs = jsonData[0]['args']
    alg = jsonDataArgs['algorithm']
    largeTrial = jsonDataArgs['trial_seed']
    subTrial = jsonDataArgs['hparams_seed']
    if alg not in data:
        data[alg] = {}
...
print('\n --- Unweighted Train Validation --- 385')
    ...
    for t in trials:
        ...
        for sub in sorted(data[a][t].keys()):
            ...
            if acc > bestAcc:
                ...
                currData = subTrialData
                bestCheckPoint = checkPoint
        bestTrialAccuracy = getEnvAccuracy(currData, 3)
        res[a].append(bestTrialAccuracy)
        resDet[a][t] = (bestIndx, currData['step'])
...
def dictToChart(d):
    ...
    for alg in ['ERM', 'IRM', 'GroupDRO', 'Mixup', 'MLDG', 'CORAL', 'MMD', 'DANN', 'CDANN', 'SagNet', 'EQRM']:
        res[0] += alg + '\n'
