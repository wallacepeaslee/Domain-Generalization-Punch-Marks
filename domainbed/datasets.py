...
class PunchMarkMultipleEnvironmentImageFolder(MultipleDomainDataset):
    '''
    Copied from MultipleEnvirontmentImageFolder, with a different augment_transform
    '''
    def __init__(self, root, test_envs, augment, hparams):
        super().__init__()
        environments = [f.name for f in os.scandir(root) if f.is_dir()]
...
class PunchMarksDefault(PunchMarkMultipleEnvironmentImageFolder):
    N_STEPS = 200
    CHECKPOINT_FREQ = 2
    N_WORKERS = 1
    def __init__(self, root, test_envs, hparams):
        dataset_filepath = '' #TODO: add relevant dataset filepath
        dataset_folder_name = '' #TODO: add relevant folder name
        self.dir = os.path.join(dataset_filepath, dataset_folder_name)
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
        
class PunchMarksSweep(PunchMarkMultipleEnvironmentImageFolder):
    N_STEPS = 400
    CHECKPOINT_FREQ = 5
    N_WORKERS = 1
    def __init__(self, root, test_envs, hparams):
