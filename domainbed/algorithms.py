...
class ERM_unfreeze(Algorithm):
    """
    Empirical Risk Minimization (ERM), where batch norms are not frozen
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer_unfreeze(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
            num_classes,
            self.hparams['nonlinear_classifier'])
...
class ERM_unfreeze_unpretrained(Algorithm):
    """
    Empirical Risk Minimization (ERM), where batch norms are not frozen networks are not pretrained
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer_unfreeze_unpretrained(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
            num_classes,
            self.hparams['nonlinear_classifier'])
